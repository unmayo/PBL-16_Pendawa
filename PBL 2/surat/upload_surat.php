...
$target_dir = __DIR__ . '/'; // Change this to your target directory
$uploadOk = 1;

// Get 'no_ktp' from POST data
$no_unik = isset($_POST['no_unik']) ? $_POST['no_unik'] : '';
...
foreach ($_FILES as $key => $file) {
    ...
    $newFileName = !empty($no_unik) ? $originalFileName . '.' . $no_unik . '.' . $extension : $originalFileName . '.' . $extension;
    ...
    if ($uploadOk) {
        if (move_uploaded_file($file["tmp_name"], $target_file)) {
            $filePaths[$key] = 'surat/' . $newFileName;
        } else {
